...
bool voxelmap::add_pointcloud(pcl::PointCloud<pcl::PointXYZ>::Ptr &msg) {
    //解决方案： 并行创建i个grid，并行合并相同grid
    //1.construct a new voxel map
    ros::Rate r(50);
    r.sleep();
...
//#pragma   omp  parallel for //并行验证失败
...
//bool voxelmap::isSamegrid(Eigen::Vector3d &p1, Eigen::Vector3d &p2) {
//    bool sameX = (floor(p1.x() / resolution) == floor(p2.x() / resolution));
//    bool sameY = (floor(p1.y() / resolution) == floor(p2.y() / resolution));
//    bool sameZ = (floor(p1.z() / resolution) == floor(p2.z() / resolution));
...
//        Eigen::Matrix<double, 4, 4> cov = neighbors.eval() * neighbors.transpose().eval() / (this_grid->point_count_);
...
//bool voxelmap::KNN_search(Eigen::Vector3d &point, size_t n, double range,
...
//voxelmap::voxelmap(bool near, double new_resolution = 1.0f) : nearby_(near), resolution(new_resolution) {
//    std::cout << "2" << std::endl;
//    if (nearby_) {
//        delta_near_.resize(7);
////        delta_near_.reserve(7);
...
//    }
//}
//void voxelmap::init(bool near, double res) {
//    if (!near) {
//        delta_near_.push_back(Eigen::Vector3d(0.0, 0.0, 0.0));
//        delta_near_.push_back(Eigen::Vector3d(resolution, 0.0, 0.0));
//        delta_near_.push_back(Eigen::Vector3d(0.0, resolution, 0.0));
